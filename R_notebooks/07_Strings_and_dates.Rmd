...
(as it currently is). Specifically, we want to know the maximum, minimum, and
average temperatures for each year. Create this summary table called
`weather_summary`.
...
Finally, we'll write the summary weather data to a new CSV file
`./data/weather_annual.csv`.

```{r}
readr::write_csv(weather_summary, "./data/weather_annual.csv", na = "")
```
